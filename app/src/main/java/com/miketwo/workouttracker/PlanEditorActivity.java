...
public class PlanEditorActivity extends Activity {
    ...
    private void renderExercises(Models.Plan p){
        ...
        for(Models.Exercise e:p.exercises){
            LinearLayout card=Ui.card(this);card.addView(Ui.exerciseHeader(this,e.name,index+". "+e.name,21));String target=e.durationSeconds>0?e.sets+" sets × "+e.durationSeconds+" seconds":e.sets+" sets × "+e.reps+" reps @ "+WorkoutMath.formatWeight(e.weight)+" "+e.unit;card.addView(Ui.text(this,target+(e.muscleGroup.isBlank()?"":"  •  "+e.muscleGroup),16,Ui.MUTED));if(!e.notes.isBlank())card.addView(Ui.text(this,e.notes,14,Ui.MUTED));
            final int original=index-1;Button drag=Ui.smallButton(this,"⇅ Drag to reorder");drag.setOnTouchListener(new android.view.View.OnTouchListener(){float down;public boolean onTouch(android.view.View v,android.view.MotionEvent event){if(event.getAction()==android.view.MotionEvent.ACTION_DOWN){down=event.getRawY();return true;}if(event.getAction()==android.view.MotionEvent.ACTION_UP){int move=event.getRawY()>down?1:-1;int target=Math.max(0,Math.min(p.exercises.size()-1,original+move));if(target!=original){java.util.Collections.swap(p.exercises,original,target);db.reorderExercises(planId,p.exercises);render();}return true;}return true;}});card.addView(drag);Button delete=Ui.smallButton(this,"Remove");delete.setOnClickListener(v->new AlertDialog.Builder(this).setMessage("Remove "+e.name+" from this plan?").setNegativeButton("Cancel",null).setPositiveButton("Remove",(d,w)->{db.deleteExercise(e.id);render();}).show());card.addView(delete);exerciseList.addView(card);index++;
        }
